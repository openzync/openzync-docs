...
>
> **Document Status:** Final | **Last Updated:** 2026-06-05 | **Total Duration:** 22 weeks (8 phases)
> **Original SRS estimate:** 14 weeks (6 phases) — recalibrated to **22 weeks** (+57%) based on production-grade rigour.
...
|--------|-------------|---------|-------|
| Total duration | 14 weeks | **22 weeks** | +8 weeks |
| Phases | 6 | **8** (split Phase 5) | +2 |
| Person-weeks (est.) | ~84 | **~200** | +116 |
...
### Why the +8 Week Delta?

| Factor | Weeks Added | Detail |
|--------|------------|--------|
| Phase 1 was 3x overcommitted | +2 | 6 interdependent features in 2 weeks → 4 weeks |
| Phase 5 was impossible | +3 | 80% coverage + Go SDK + load test + security audit + docs in 2 weeks → split into 5a+5b = 5 weeks |
...
| Graphiti integration risk buffer | +0.5 | Third-party library, API stability concerns, bug workarounds |
...
(2 wks)    (4 wks)     (4 wks)     (3 wks)     (4 wks)     (3 wks)      (2 wks)
...
**Track C — Graph DB & Embeddings (shared)**

| Week | Task | Est. Days | Depends On |
|------|------|-----------|------------|
| W2 | Graphiti integration: version pinning (`graphiti-core==0.29.1`), async wrapper, circuit breaker scaffold | 3 | 0.3 |
| W2 | `EmbeddingClient`: provider abstraction (OpenAI/Ollama), dimension validation, batch embedding | 2 | 0.3 |
...
| **G0.5** | Embedding client connects to OpenAI + Ollama. Dimension mismatch raises `ConfigurationError` | Unit test + integration |
...
## Phase 2 — Full Feature Parity (Weeks 7–10)

**Theme:** *"SDK published to PyPI, MCP server works in Claude Desktop, graph queries performant, community summaries generated."*

> **Original estimate:** 3 weeks → **Corrected: 4 weeks.** Python SDK with sync/async duality + full test suite + PyPI publishing is 1.5 weeks. MCP server with 8 tools + auth + stdio/SSE is another 1+ weeks. These cannot run in parallel with graph query endpoints and community summarisation — they all depend on the same stable API surface.
...
| Week | Task | Est. Days | Depends On |
|------|------|-----------|------------|
| W7 | Graph query endpoints: `GET /nodes`, `GET /nodes/{id}`, `GET /edges`, `DELETE /nodes/{id}`, cursor pagination | 4 | 0.9 (Graphiti) |
| W7 | Graph client abstraction: `GraphBackend` interface, FalkorDB + Neo4j implementations, `org_id` enforcement | 3 | 2.4 |
| W8–9 | **MCP server**: stdio + SSE transports, 8 tools (add_memory, get_context, search_memory, add_fact, list_facts, get_user_graph, create_user, list_sessions), auth, Claude Desktop config | 5 | 2.1 |
| W9–10 | **Community summarisation**: Louvain/Label Propagation algorithm, LLM summary generation, nightly schedule | 5 | 2.4 |
| W10 | Full-text search: `pg_trgm` GIN indexes, combined with pgvector + graph in RRF | 2 | 2.6 |
...
| W9 | Grafana dashboards: 6 provisioning panels (API RED, context latency, queue depth, token usage, graph growth, service health) | 2 | 2.8 |
| W9–10 | Helm chart scaffold: `Chart.yaml`, `values.yaml`, deployment templates, services, PVCs | 3 | 2.9 |
...
## Phase 3 — NLP Enrichment (Weeks 11–13)
...
| Week | Task | Est. Days | Depends On |
|------|------|-----------|------------|
| W11 | Dialog classification pipeline: prompt (`classify_dialog_v1.jinja2`), LLM structured output, configurable labels per org | 4 | 1.6 (entity pattern) |
| W11 | Classification eval: golden dataset (200 labeled turns), accuracy ≥85%, eval suite in CI | 2 | 3.1 |
| W12 | Structured extraction worker: org-defined JSON Schema → LLM call → jsonschema validation | 4 | 3.1 |
| W12 | Structured extraction eval: 10 schema variations, 100% schema compliance | 2 | 3.2 |
| W13 | Custom entity ontologies: per-org entity types in extraction prompts, prompt injection guardrails | 3 | 1.6 |
| W13 | Entity merge dedup worker: weekly scheduled task, LOWER(name)+org_id dedup, audit trail | 3 | 3.3 |

**Track B — Endpoints & Infrastructure (Engineer B: Mid)**

| Week | Task | Est. Days | Depends On |
|------|------|-----------|------------|
| W11 | Classification endpoints: `GET /users/{id}/sessions/{sid}/classifications` | 2 | 3.1 |
| W11 | Schema CRUD API: `POST/GET/PUT/DELETE /v1/admin/schemas` | 3 | — |
| W12 | Structured extraction query endpoint: `GET /users/{id}/sessions/{sid}/extract` | 2 | 3.2 |
| W12–13 | **LLM cost control**: per-org daily token budget, enrichment depth levels (none/basic/full), `llm_usage` table, hard cutoff (429) | 4 | 1.9 (llm_usage table) |
| W13 | PII detection pre-processor (regex + spaCy NER, configurable per org) | 2 | 1.3 |
| W13 | Cost projection dashboard: Grafana panel per-org token consumption, projected overrun date | 2 | 3.5 |
...
> **Exit criterion:** Cost controls operational in production — enrichment depth levels, daily token budget with hard 429 cutoff, Ollama as default backend, cost per step tracked in `llm_usage` + Grafana dashboard.

### Teaching Sessions (Before Phase 3)

| Day | Session | Duration | Led By |
|-----|---------|----------|--------|
| W11 D1 | Dialog classification + structured extraction architecture | 60 min | Tech lead |
| W11 D1 | Eval methodology for NLP (golden datasets, regression testing, thresholds) | 60 min | Tech lead |
| W11 D2 | PII detection & guardrails | 45 min | Security lead |
| W11 D3 | LLM cost control patterns | 45 min | Tech lead |

---

## Phase 4 — Dashboard, Production Infra & TypeScript SDK (Weeks 14–17)
...
| Senior infra/backend | Track B (production infra + TS SDK) | 100% |
...
| Week | Task | Est. Days | Depends On |
|------|------|-----------|------------|
| W14 | Next.js 14 setup: App Router, auth (JWT + refresh), auto-generated API client from OpenAPI, shadcn/ui + Tailwind | 3 | P0 (JWT) |
| W14–15 | Tenant management UI: org CRUD, API key generate/revoke (show-once modal), quota configuration | 4 | 4.1 |
| W15–16 | User graph explorer: Cytoscape.js integration, entity visualisation, search, zoom/pan | 5 | 2.4 |
| W16–17 | Analytics panels: usage charts from Mimir queries, API stats, token usage, worker queue depth | 4 | 2.8 (Grafana) |

**Track B — TypeScript SDK & Production Infra (Engineer B: Senior)**

| Week | Task | Est. Days | Depends On |
|------|------|-----------|------------|
| W14 | **TypeScript SDK**: fetch-based (browser + Node.js), typed generics, tree-shakeable, npm publish | 5 | Phase 2 stable API |
| W15 | Production Docker Compose finalisation: pgBouncer tuning, Sentinel failover test, Traefik TLS | 3 | 2.7 |
| W16–17 | **Kubernetes Helm chart**: Deployment + HPA (API CPU>70%, Worker queue depth), PDB (minAvailable=1), PV (PG 100Gi, FalkorDB 50Gi, Redis 10Gi), NetworkPolicies (deny-all + allow inter-service), External Secrets Operator | 6 | 4.3 |
| W17 | `CORS_ORIGINS` env var, CSRF protection for dashboard, rate limit on login | 1 | 4.1 |

**Track C — DevOps (Engineer C: DevOps)**

| Week | Task | Est. Days | Depends On |
|------|------|-----------|------------|
| W14–15 | CI/CD: full pipeline (lint → unit → integration → build → e2e → security → sdk → publish) | 3 | 2.7 |
| W15 | Prometheus adapter for worker HP A (custom metric exposing queue depth) | 2 | 2.8 |
| W16 | Grafana dashboards auto-provisioned via JSON + datasource YAML | 2 | 2.8 |
| W17 | OpenTelemetry tracing: HTTP → ARQ → LLM → Graphiti, Tempo ingestion, log↔trace correlation | 2 | 2.8 |
| W17 | Alert rules: 8 rules (error rate >1%, p99 latency >2s, queue depth >1000, failed task rate >5%) | 1 | 4.7 |
...
| **G4.7** | Unit test coverage ≥ 78% on all backend packages | `pytest --cov` |
...
| Day | Session | Duration | Led By |
|-----|---------|----------|--------|
| W14 D1 | Next.js 14 App Router + auth flow | 60 min | Senior full-stack |
| W14 D1 | shadcn/ui + Tailwind conventions | 30 min | Senior full-stack |
| W14 D2 | Auto-generated API client from OpenAPI | 45 min | Senior dev |
| W14 D3 | TypeScript SDK design patterns | 60 min | Senior dev |
| W15 D1 | Kubernetes Helm chart basics | 90 min | DevOps |
| W15 D2 | Grafana dashboard as code | 45 min | DevOps |

---

## Phase 5a — Hardening: Tests & Observability (Weeks 18–20)

**Theme:** *"Everything tested, monitored, and secure. Load test passes at 500 req/s."*

> **Original estimate:** Phase 5 was 2 weeks total → **Now split: 5a (3 weeks) + 5b (2 weeks) = 5 weeks.** The original jammed 80% test coverage, integration suite, load test, security audit, Go SDK, and release docs into 2 weeks — impossible. Phase 5a focuses on test coverage, load testing, security audit, circuit breakers, and PII redaction.
...
| Week | Task | Est. Days | Depends On |
|------|------|-----------|------------|
| W18 | Unit test coverage push: identify <80% packages, add service tests, edge case coverage | 5 | All phases |
| W18 | Integration test suite: P0 + P1 endpoints, testcontainers (PG + Redis + FalkorDB), mock LLM | 5 | 5.1 |
| W19 | **Cross-tenant test matrix**: 3 tenants × 6 resources × 3 access methods = 324 parameterized tests | 2 | 5.1 |
| W19 | Security audit: SQL injection, auth bypass, cross-tenant, dependency scan (pip-audit), SAST (bandit + semgrep) | 4 | 5.2 |
| W20 | Security findings remediation (blocker: fix all critical/high) | 2 | 5.3 |

**Track B — Production Hardening (Engineer B: Senior)**

| Week | Task | Est. Days | Depends On |
|------|------|-----------|------------|
| W18 | **Circuit breakers**: LLM API, embedding API, FalkorDB, Redis — `pybreaker` with per-org isolation | 3 | 0.8 |
| W18 | Retry audit: every external call has `timeout + retry + circuit breaker + fallback` | 2 | 5.5 |
| W19 | **Orchestrator migration**: event-driven → single orchestrator for enrichment pipeline | 3 | 1.8 |
| W19 | Graceful degradation: context assembly without graph, ingestion without enrichment | 2 | 5.6 |
| W20 | PII redaction pipeline: pre-LLM regex + spaCy NER, configurable per org, blocking mode (422) | 3 | 5.7 |
| W20 | Rate limiting rollout: per-key config via admin API, `X-RateLimit-*` headers, 429 body | 2 | 0.8 |

**Track C — Load Test & Monitoring (Engineer C: DevOps)**

| Week | Task | Est. Days | Depends On |
|------|------|-----------|------------|
| W18–19 | **Load test**: 500 req/s sustained × 5 min, hardware = 4 CPU/8GB API + 2 CPU/4GB worker + separate load generator | 4 | Phase 4 infra |
| W19 | Performance bottleneck diagnosis + fix (if load test fails) | 3 | 5.10 |
| W20 | Performance benchmark suite: repeatable, CI-scheduled, regression detection | 2 | 5.11 |
| W20 | SLO dashboards: 99.9% uptime, p99 <300ms, error budget, burn rate alerts | 2 | 4.7 |
| W20 | On-call runbooks: 5 runbooks (API outage, DB failover, worker backlog, LLM outage, security incident) | 2 | — |
...
## Phase 5b — Release Readiness (Weeks 21–22)

**Theme:** *"Go SDK published. Air-gapped works. All docs finalised. v1.0.0 released."*

> **This is the buffer phase.** Items deferred from earlier phases get completed here. If earlier phases ran over, this phase gets compressed — but NEVER cut the Go SDK or air-gapped verification.

### Team
| Role | Person | Allocation |
|------|--------|------------|
| Senior (Go) | Track A (Go SDK) | 100% |
| Mid backend | Track B (bi-temporal + docs) | 100% |

### Tasks by Track

**Track A — Go SDK (Engineer A: Senior)**

| Week | Task | Est. Days | Depends On |
|------|------|-----------|------------|
| W21 | Go SDK scaffold: interface-based design, functional options, context propagation, all 5 domains | 5 | Phase 2 stable API |
| W21 | Go SDK: retry + error handling + pagination iterator | 3 | 5b.1 |
| W22 | Go SDK: publish to `pkg.go.dev`, CI pipeline, integration tests | 2 | 5b.2 |

**Track B — Migration & Documentation (Engineer B: Mid)**

| Week | Task | Est. Days | Depends On |
|------|------|-----------|------------|
| W21 | **Bi-temporal upgrade**: `facts_history` table, `FOR SYSTEM_TIME AS OF` queries, migration runbook for 500K+ facts | 4 | 1.8 (facts) |
| W21 | Air-gapped deployment: Docker image tarballs (11 images, ~2.6GB), Ollama model distribution (~2.3GB), private PyPI/npm/Go mirrors | 3 | 4.4 |
| W22 | Admin dashboard: ARQ job monitor (queue depth, success/failure count, DLQ inspection) | 3 | 5.6 |
| W22 | Release artifacts: CHANGELOG (keepachangelog format), CONTRIBUTING.md (PR workflow + standards), LICENSE (Apache 2.0), README.md with quickstart | 2 | — |
| W22 | Final security sweep: full SAST + DAST + dependency scan + secrets scan | 2 | 5.3 |
| W22 | Release v1.0.0: tag, CI publish to PyPI + npm + pkg.go.dev + Docker Hub + GitHub | 1 | 5b.4 |
...
| **G5b.5** | All release artifacts published: PyPI, npm, pkg.go.dev, Docker Hub | Verified |
| **G5b.6** | Overall unit test coverage ≥ 80% | `pytest --cov` |
| **G5b.7** | All implementation docs match code (final audit) | `doc-audit` CI stage |
| **G5b.8** | v1.0.0 tagged and released | GitHub release page |

### Release Go/No-Go

**ALL of these must be true to release v1.0.0:**

1. All 42 quality gates (G0.1–G5b.8) pass
...
| P2 — Full Parity | 7–10 | 2.5 | SDK, MCP, graph queries, communities | Graph query latency at 10k+ nodes | 10% |
| P3 — NLP Enrichment | 11–13 | 2 | Classification, extraction, cost control | LLM cost blowout | 10% |
| P4 — Dashboard & Infra | 14–17 | 3 | Dashboard, Helm, production infra | Helm chart on real cluster | 10% |
| P5a — Hardening | 18–20 | 2.5 | Tests, load test, security, circuit breakers | Load test failure at 500 req/s | 5% |
| P5b — Release | 21–22 | 2 | Go SDK, bi-temporal, air-gapped, release | Go SDK quality vs timeline | 5% |
...
| No Go SDK | P2 | P5b | Go ecosystem users cannot integrate |
...
4. **FalkorDB-first, validate Neo4j weekly.** Run integration test suite against both backends at least weekly to catch regressions early.
...
| **Phase 4 → Phase 5a** | All 7 G4 gates pass. Helm chart deploys on 3-node cluster. Dashboard usable. |
| **Phase 5a → Phase 5b** | All 9 G5a gates pass. Load test: 500 req/s × 5 min, all criteria met. Security: 0 critical/high. |
| **Phase 5b → Release** | All 8 G5b gates pass. All 42 quality gates across ALL phases passing. Tech lead + Security sign-off. |
